...
"""
PI-STDNet V8.3 严谨消融与基线对比实验脚本
====================================
集成了满血版、各消融组，以及 SE-Net / CBAM 经典注意力基线的公平对比。
"""
...
# 🌟 新增：注意力基线选择开关
parser.add_argument('--attn_type', type=str, default='physics', 
                    choices=['physics', 'se', 'cbam'],
                    help='选择注意力机制：physics (默认), se, 或 cbam')
...
CONFIG = {
    ...
    'USE_AMP': True,
    'LEARNING_RATE': 5e-4, 'WEIGHT_DECAY': 0.05,
    'NUM_EPOCHS': 15, 'WARMUP_EPOCHS': 5,
...
logger.info(f"🧪 实验: {args.exp_name} | 注意力类型: {args.attn_type.upper()}")
...
class MultiScalePhysicsAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        hv = (dbz_max > 0.27).float()
        return torch.sigmoid(self.fusion(torch.stack(frame_feats, dim=0).max(dim=0)[0])) * hv, hv

class SENetAttention(nn.Module):
    def __init__(self, n_frames=4, channels_per_frame=11, reduction=16):
        super().__init__()
        self.n_frames, self.cpf = n_frames, channels_per_frame
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        dbz_frames = [x[:, t*self.cpf : t*self.cpf+1] for t in range(self.n_frames)]
        hard_valid = (torch.stack(dbz_frames, dim=1).max(dim=1)[0] > 0.15).float()
        se_weight = self.fc(self.global_pool(x).view(B, C)).view(B, C, 1, 1)
        attn = torch.sigmoid(self.spatial_proj(x * se_weight)) * hard_valid
...
class CBAMAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        dbz_frames = [x[:, t*self.cpf : t*self.cpf+1] for t in range(self.n_frames)]
        hard_valid = (torch.stack(dbz_frames, dim=1).max(dim=1)[0] > 0.15).float()
        avg_feat, max_feat = self.avg_pool(x).view(B, C), self.max_pool(x).view(B, C)
        x_ca = x * torch.sigmoid(self.channel_mlp(avg_feat) + self.channel_mlp(max_feat)).view(B, C, 1, 1)
...
class PI_STDNet_Ablation(nn.Module):
    def __init__(self):
        ...
        if self.use_physics_attn:
            if args.attn_type == 'physics':
                self.physics_attn_module = MultiScalePhysicsAttention(CONFIG['N_FRAMES'])
            elif args.attn_type == 'se':
                self.physics_attn_module = SENetAttention(CONFIG['N_FRAMES'], CONFIG['CHANNELS_PER_FRAME'])
            elif args.attn_type == 'cbam':
                self.physics_attn_module = CBAMAttention(CONFIG['N_FRAMES'], CONFIG['CHANNELS_PER_FRAME'])
        ...
        cls_in_dim = dims[3]
        if self.use_topk: cls_in_dim += dims[1]
        else:
            self.gmp_fallback = nn.AdaptiveMaxPool2d((1, 1))
            cls_in_dim += dims[1]
        if self.use_rot_stats: cls_in_dim += 128
    ...
    def forward(self, x):
        ...
        feat_high = skips[3].mean(dim=(2, 3))
        if self.use_topk: 
            feat_low = self.topk_pool(skips[1], physics_attn)
        else: 
            feat_low = self.gmp_fallback(skips[1]).view(B, -1)

        # 检查是否打上了“train.py”的标记
        if getattr(self, 'use_train_py_order', False):
            # 给 PI-STDNet (Ours) 用的顺序
            cls_features = [feat_low, feat_high]
        else:
            # 给旧版 Baseline/SE/CBAM 用的历史顺序
            cls_features = [feat_high, feat_low]
...
class EMA:
    def __init__(self, model, decay=0.9995):
        self.model, self.decay = model, decay  # ✅ 加上 ", decay"
