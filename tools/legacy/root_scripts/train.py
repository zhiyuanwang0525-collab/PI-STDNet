...
file_handler = logging.FileHandler("v8.2_server_training_GWP.log", mode='a', encoding='utf-8')
...
class PI_STDNet_V8(nn.Module):
    def __init__(self):
        ...
        self.gmp_pool = nn.AdaptiveMaxPool2d((1, 1))  # 替换TopK为GMP
    ...
    def forward(self, x):
        ...
        cls_logit = self.cls_head(torch.cat([self.gmp_pool(skips[1]).view(x.shape[0], -1), skips[3].mean(dim=(2, 3)), self.rot_proj(rot_stats)], dim=1)).squeeze(-1)
...
def main():
    ...
    for epoch in range(CONFIG['NUM_EPOCHS']):
        ...
        if wc_csi > best_wc_csi:
            ...
            ema.apply_shadow()
            torch.save({'model_state_dict': model.state_dict(), 'nc_csi': nc_csi, 'wc_csi': wc_csi}, "best_pi_v8_GWP.pth")
            ema.restore()
